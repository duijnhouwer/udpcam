function [p,pressedOk] = guisetstruct(p,prompt,maxParmsPerDlg)
    ...
    parms_display=cellfun(@(x)strrep(x,'_',' '),parms,'UniformOutput',false);
        

    [answer] = inputdlg(parms_display,strtrim(prompt),[nrLines ones(size(nrLines))*numel(prompt)],current,options);
